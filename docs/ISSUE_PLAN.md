...
**Status:** `tools/camera_calibration.py` (a copy of the canonical calibration script from this project's separate Georeferencing codebase, not published on GitHub) computes + saves intrinsics (`K`/`D`). `tools/camera_calibration_legacy.py` and `tools/generate_calibration_chessboard.py` are deprecated (see notices at top of each file). Gap is extrinsic collection and a guided workflow doc.
