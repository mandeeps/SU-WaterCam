...
def save():
    ...
    # call hourly_save if run at least once to calculate rate of change
    global DF
    ...
    avg_file = os.path.join(DIRNAME, 'data/thermal_sensor_record.csv')
...
def hourly_rate(DF, avg):
    ...
    # calculate mean of rate of change per pixel over total time tracked
    change = []
    for column in change_per_hour: # multiply by 100 and round to single decimal
        # to make values more human-readable
        change.append((change_per_hour[column].mean()*100).round(1))
    ...
    for column in change_per_hour:
        if change_per_hour[column] < median:
            land_or_water[column] = True # water
        else:
            land_or_water[column] = False # land
